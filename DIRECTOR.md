...
### 工单十三：会话列表异步化（小模型机械单，四小步）—— 🟡 代码验收通过（2026-09-11，见归档验收记录）；补刀后待用户实测
...
### 工单十三补刀：分块读多字节字符骑线修复 —— ▶️ 待施工（2026-09-11 验收实验发现）
...
> 完整清单见 归档.md「八、后续排队」。大盘子备忘：**面板内多标签并行会话**（架构级，
> 用户已提出，09-09 降级为按需——用户洞察：实际业务并行需求少；设计草案在 HANDOVER.md
> 「下一个大功能」，工单四已埋会话域持有预留，落地需专项会话）。
