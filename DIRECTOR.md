...
>
> **壳引擎边界（用户拍板 2026-09-14，工单十三七连改教训）：pi 公开导出的能力，壳不自研。**
> SessionManager、官方 SDK API 等 pi 已公开导出的能力，壳只消费接口、不二次实现；
> pi 拥有的知识（会话格式、事件语义），壳不复制第二份。签工单前先 grep pi 公开导出，
> 能调 API 的不写解析。架构铁律管“不绑 VS Code”，本条管“不重造 pi”——两条合起来
> 才是完整的壳。
>
