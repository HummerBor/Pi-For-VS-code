...
市面上的方案各有各的沉：**Codex** 一跑电脑就卡；**Claude Code** 费 token，切模型还得配 cc-switch 这类代理；**Hermes、DSH、OpenClaw** 也都试过，个个庞大，调用麻烦。pi 不一样——轻、透明、没黑盒，那就在它身上搭个面板：省 token、省内存，主题想怎么改怎么改。
