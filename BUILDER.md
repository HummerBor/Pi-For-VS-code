...
- **症状**：终端 `pi -r`/`pi -c` 秒开，插件启动卡十几秒；两个 VSC 窗口都慢。
- **根因（探针实锤）**：`loadPiSdk()` 动态 import 的是 pi 包**库入口 dist/index.js**——
  884 个文件的模块图，冷缓存/高负载下光读盘+杀软扫描就 2~16s（探针实测冷 15.7s /热 1.2s）。
  终端 pi 秒开正因它跑的是**单文件 bundle**（dist/bundle/cli.js）——一文件 vs 八百文件。
  **不是版本倒退**：这跳从第一天就有，被双窗口/多进程负载放大后显形（用户「这块之前都
  是快的」）。
- **修法**：loadPiSdk 优先 `dist/bundle/index.js`（单文件 bundle，实测 257ms / API 面零
  缺项），旧版包无 bundle 回退库入口（required 校验兜兼容）。探针固化在
  scripts/probe-boot-timing.mjs（另证实：sdk.listModels 不存在/建服务 135ms——网络理论
  与代理理论均排除）。
- 回归面：compile ✅；四套用例 ✅；probe-boot-timing：loadPiSdk 257ms。
- 实测点：重启两个 VSC 窗口 → 插件启动应秒回（不再卡十几秒）。
- 机器侧可选项（非代码）：给 pi 包目录加 Defender 排除可再压冷启动；另「D 开错页签」
  （聚焦语义 lastSpoke 修正）仍待下一刀。
- **版本号口径（2026-09-23 用户拍板）**：`npm run ship` 跟的是**线上版本线**（从市场版
  bump，发布号只增不复用）；本地 `npm run package` 的测试包号**随便**，不参与递增纪律。
  发布流归 ship 所在会话管（当日 `chore: release` 并行提交即发布线），施工会话只交代码。

# K 刀：关标签转移漏 boot（2026-09-22，D 实测实锤，激活不变量收口）

- **症状**：重启后关掉活动页签，转移到的恢复页签永远空显（欢迎页 + 页脚「— 思考 —」），
  再点它也没反应（双重早退）；会话文件无损（只是没渲染），切走再切回才能救。
- **根因（A 刀日志实锤 13:09:42）**：`tabClose t14 → tabSwitch t12 needState=true → 只有
  out uiState arr(0)、无 boot start`——恢复页签的「首次激活必须 boot」只挂在
  handleTabSwitch 尾巴；关标签转移直改 activeTabId，webview 随后的 tabSwitch 又被
  「已活动」早退挡掉；needState 只拿到空快照 → 核心从未起进程 → 永远空白。
- **修法**：激活不变量收口 `bootRestored(tabId)` 一处——handleTabSwitch 与 handleTabClose
  转移路径都过它（凡「页签成为活动」必须 boot）；空快照兑底对齐 F′ 口径（启动中不发空快照，
  只对真·空页签回原子空快照）。
- 回归面：compile ✅；四套用例 ✅（宿主侧无 webview，探针面不适用）。
- 实测点：重启后关掉活动页签 → 转移到的页签**立即显示它的会话内容/页脚模型**；连续关转也正常。
  **✅ 用户实测通过（2026-09-22 收官）**。
- 附注（设计现状非 bug）：未 boot 的恢复页签在页签栏显示**存档标题**（可能是 J 刀前的 UTC
  时间串），首次激活后随 state 重算为本地时间/会话名——存档标题是「重启后页签栏可读」的
  必要品，不改。
...
- **①用户文案收口**：sessionMode 描述砍成「启动时如何接续会话（修改后重启生效）」——
  「进程内直连/审计红线/判死」是开发者语境（留代码注释 + AGENTS.md），不进用户视野；
  i18n sSessionModeDetail 同步回短版。用户只关心设置干什么。
- **②空会话不再显时间戳**：新建页签「新会话」→ 时间 的跳变不对——pi 同口径「没内容的
  会话没文件」，标题保持「新会话」直到有内容（有内容 pi 自动生成会话名）。判据复用
  H 刀：文件在 ⇔ 有内容（fmtSessionTitle 里 existsSync 单点收口）。
- **③时区 bug（用户抓到）**：会话文件名时间戳是 UTC（尾缀 Z），此前裸读字符串不转时区
  ——差 8 小时（20:00 建的会话显 12:00）。panel.fmtSessionTitle + webview fmtSession
  双端经 Date 转本地显示。
- 页头改镜像宿主 tabs 标题（唯一事实源，webview 只渲染）：空会话/时区判定单点收口在
  宿主，webview fmtSession 降级为 tabs 未到时的兜底（handleTabs/applyState 双入口刷新）。
- 回归面：compile ✅；四套用例 ✅；probe-live-merge ✅（webview 改动，合并行为输出不变）。
- 实测点：①设置描述一句短文案②新建空页签标题保持「新会话」，发首条消息后变会话名
  ③页签/页头时间戳 = 本地时间（与创建时刻对得上）。
- 队列：F′ ✅ → E ✅ → C ✅ → I ✅ → **J ✅（本刀）** → G 闸门挪位 → B 迁移重做 → D 重启聚焦。
...
**类名：「pi 启动态从未被显式建模」**——三条缝同一个根：
①**真值必达无保障**（F′+G）：boot 链 refreshState 撞上 piClient init 未完成时，collectState
守卫「不等就回 null」→ state 真值静默丢失、此后无人再刷 → 页脚永久「— 思考 —」/页头永久
「临时(未保存)」（日志实锤：左窗 t24/25/26 只有 render 无 state 全军覆没；右窗 t64 init
及时则全好——纯竞态，谁赶上 init 翻真谁赢）；
②**占位冒充终值**（E）：启动窗口把「临时(未保存)」当终值显（拍板 3 要显「启动中」）；
③**启动期乱灌默认值**（H✅/C/B）：-c 把最近会话强加给空页签、影子 "_" 串页签、迁移跑在
boot 链一炸全死（P0）。
**统一修法原则：「启动中」显式建模 + 真值必达 + 启动期不引入任何默认值。**

**队列按因果重排**（原 B 挪后）：F′ 真值必达（根因刀）→ E 占位 → G 闸门挪位 → C 去影子 →
B 迁移重做 → D 重启聚焦。一刀一验（编译+用例+用户实测）不变。

**F′ 施工回报（根因刀，宿主侧零协议变更）**：
- collectState 守卫改「只判 client 在不在」：getState 内部 await ready()，init 未完成就等，
  不再静默跳过真值（根因修）；
- needState 对启动中页签不发空快照（0.1.42 修法取回）：有 client 要真值快照（内部等
  init），没 client 才是真·空页签回原子空快照——占位不再被钉死；
- boot 链尾终值快照（0.1.42 修法取回）：void postUiState() 一次补齐页脚/标题数据源，
  消灭「无人再刷」；
- boot 失败必弹错（拍板 4）：catch 发 notice（opFail+err）+ dbg FAIL 双保险，吞掉语义
  保留（不重抛）。
- 回归面：npm run compile ✅；test:detail / test:subagent / test:revert ✅；宿主侧改动
  无 webview，探针面不适用。
- 实测点：冷窗口新建页签 → 页脚秒级显示模型/思考（不再永久「—」）；页头从「临时(未保存)」
  变为会话时间戳标题。

# H 刀提前：空页签/无记忆页签禁 -c + 不入库（2026-09-22，用户实测 A 刀时抓到现行，按拍板提前切）

- **A 刀实测结论：通过（实证）**——用户实测当场复现「空页签重启后变成历史第一条/之前点过的
  页签」，A 刀日志逐条抓到全过程（pi-chat-debug.log 10:31:23–53Z：t56 空页签 boot 干净 →
  窗口重启 t56 被页签栏恢复 → 点 t54 发现两个问题 → 点回 t56 → boot 无 freshTab → -c →
  216 条旧会话整包灌入）。诊断日志首战即定位，A 刀目的达成。
- **根因（用户诊断 + 日志/磁盘物证双确认）**：①空会话磁盘上无文件（pi -r 同口径：没内容
  不落盘，文件名时间戳=创建时刻、内容首条消息才写；t56 全程无文件、t57 你好123 有）；
  ②插件页签栏却把空页签持久化（当年「新标签立刻入册」防丢设计）；③重启/切到该页签 boot
  时无有效记忆 → 落到 `-c`（continueRecent）→ 把「最近一条会话」强加给空页签（= 之前点过的
  页签/历史第一条），还和原页签双写同一文件。
- **修法（两道守卫同一判据：记忆文件在 ⇔ 有内容）**：①ensureClient 仅当记忆文件存在才
  传 `-c`，否则 `[]`（SessionManager.create 新会话）；②saveTabBar/restoreTabBar 按同一
  判据过滤空页签（用户拍板：没有内容的新会话重启时既不展示也不记录），SavedTabBar 增 seq
  水位——被滤掉的页签 id 重启后绝不复用（其记忆键可能已被污染指向别人的会话文件，
  复用 = 新页签继承脏记忆）。会话记忆查询抽成 piCore.sessionMemoryFor 单一事实源
  （boot 判定/页签持久化同口径，防双实现漂移）。
- 回归面：npm run compile ✅；test:detail / test:subagent / test:revert ✅；纯宿主侧改动
  无 webview，探针面不适用。已污染页签：关掉重建即可（旧脏记忆随 id 不复用失效）。
- 队列现状：A ✅（实证）→ **H ✅（用户实测通过）**（原 H 由本刀覆盖）。队列其后重排，
  见上「启动态收口专项」节。
  注：同日并行会话已提交 36b2c5f（直令修复「点暂停清空输入内容」，piCore 两刀）；本刀
  已逐 hunk 核对归属（工作区未提交改动均为本刀），单写方纪律照旧。

# 直令修复：点暂停把刚输入的内容清空（2026-09-22，piCore 两刀，用户实测通过）
...
- **验证**：npm run compile ✅（含 tsc）；test:detail / test:subagent / test:revert ✅；
  无 webview 改动。**用户实测通过（2026-09-22）**：忙时发消息排队 → 点停止 → 排队条保留、
  现场不被抹。
...
**改动面（单笔提交，仅 scripts/publish.js gitCommit）**：版本号 bump 已随台账提前入库时
（工单31 验收 adb4fb9 收编 0.1.36），ship 的 commit 天然无变更 exit 1——增加「nothing to
commit」检测：不视为失败继续 push/publish；真失败（索引锁/权限）照旧拦截退出。spawnSync
从 stdio:inherit 改 encoding 捕获输出以做文本检测（兼容行为不变）。

**验证**：node --check 语法过；非交互闸门拦截正常；发版链路重跑见 ship 输出。
...
**→ 2026-09-20 用户实测通过（三项全过）**。

# 工单29 施工回报：扩权四项（2026-09-20，用户直令 + 原单，待实测）

**扩权口径（用户直令，越过原单 500 字阈值）**：①完成一个工具收一个，不等 settled 统一收；
②思考块限高滚动；④连续同名工具折叠成组且**不设数量上限**（bash ×50 也一行，用户拍板）；
③长 OUT 截断提示+点开全量（原单保留）。用户另给方案约束：**尽可能省内存**。

**改动面（单笔，webview/main.ts + webview/style.css + src/i18n.ts，piCore/protocol 零改动）**：
- ②：`.think-body { max-height:240px; overflow-y:auto }`（live/历史同 class 两路受益）；
  appendThink 加一行 scrollTop 滚底，零监听器
- ①③：toolEnd 重写——完成即收起（wasOpen 保持展开逻辑作废）；OUT 预览仍 1000 字，
  全量**只持引用不复制**（ref.full），首次点开才写 DOM 进 tb-full 滚动容器（320px），
  换完置 null 释放；截断提示走 title+i18n 新 key toolTruncated（中英两处）
- ④：连续同名折叠成组——DOM 相邻判定（row→box→row→box 首尾相接），中间隔文本/思考块不合；
  合并 = moveChild 节点搬移零复制 + 删两个节点 + 存活行挂 t-count ×N；被合并方全量先展开
  进隐藏盒再释放；renderAll 显式清 lastToolGroup（相邻判定本身也兕底）
- 省内存总账：零新增常驻监听器；大文本只持引用不复制、首展即弃、settled 整树重绘全清
  （内存暂态，上限 = 一个回合工具数）；④反而减少 DOM 节点

**验证**：npm run compile 全链绿；0.1.27 已打包待实测。

**待实测（用户主验收）**：①流式中工具完成一个收一个，点行可展开/再收；②读大文件（>1000 字）
行 hover 有截断提示，点开看全量可滚动，反复切换正常；③长思考流式期间限高滚动、新内容可见；
④连续 bash/edit 折成 bash ×N 一行，中间隔文本则不合并；点开组盒可见各行输出；
⑤settled 后历史重绘不回退（80 字摘要 + ×N 分组照旧）。

**记档（已解决，随 0.1.28 返工笔）**：流式期间工具行之间的空圆角色条 = 纯工具回合里
liveEnsure 为每次 toolCallStart 建的 assistant bubble 空壳（prow 清了 bubble 留在 root）；
toolStart 时一并移除（只删零文字且无 img 的空壳，回答 bubble 不可能误删）。

# 返工两笔（2026-09-20 用户实测反馈 + 直令，待实测）

**返工一（工单29扩权追修）——同名成组「会话结束才合并」根因**：空占位 bubble 插在
上一工具盒与下一工具行之间，破坏 DOM 相邻判定 → live 合不上；修法 = toolStart 时移除
空壳 liveDiv（同一根因顺带灭掉空色条穿帮）。折叠边界钉死：工具块完成即收；思考块不折
叠只限高滚动；结论回答（assistant 正文 bubble）永不折叠。

**返工二（工单28追加，用户直令）——sticky 限两行**：浮动信息面积太大，默认
max-height 56px（两行）+ overflow hidden + cursor:pointer；点击展开/再点折叠
（sticky-open toggle，addUser 挂）；迁移时旧 bubble 连 sticky-open 一起摘，新 bubble
从折叠态起步。

**验证**：npm run compile 全链绿；0.1.28 已打包待实测。

**待实测（用户主验收）**：①流式中连续同名工具实时合（bash ×N 当场变，不等回合结束）；
②工具行间空色条消失；③长提问钉住后只露两行，点击展开全量、再点折叠；④结论回答照常
全量展示不折叠。

# 思考块两修（2026-09-20 用户实测 0.1.28 反馈，待实测）

**修一：思考块滚动条「拉不上去/不能被打断」**——根因是 0.1.27 扩权加的流式自动滚底
每拍 delta 都执行 scrollTop=scrollHeight，用户往上拉一拍就被拽回。修法：贴底才滚
（追加前算 atBottom，容差 30px，用户翻上去就不再拽）。

**修二：思考块结束不折叠**——根因是 details.think 建的时候 open=true 后没人关。
修法：finalizeLive 收 thinking part 时 removeAttribute('open')——思考→正文切换、
工具块出现（toolCallStart 换 kind / toolStart 都经 finalizeLive）、settled 三条路径
全覆盖；用户手动展开仍可看全文，限高滚动照旧；历史重绘路径本来就默认收起。

**验证**：npm run compile 全链绿；0.1.29 已打包待实测。

**待实测**：①流式中往上拉思考块不再被拽回；②思考完切正文/出工具块时思考块自动收起，
点开仍可看全文。

# 0.1.30：工具组重设计 + 信息块三态折叠（2026-09-20 用户直令，待实测）

**用户新口径（对 0.1.27–29 同名成组的推翻与细化）**：①三大类 思考块/工具块/回答气泡，
思考块完成一个折一个（已做）；②工具块**不分 edit/bash** 全部合到一起成一个大折叠块——
序列中每个工具完成折一次，遇到下个信息块（思考块/正文）整组收成一行头；③回答气泡不折
叠；④user 信息块原位不折叠，钉住浮动时**超 3 行**才折，带 … + 展开/收起 chip。

**改动面（webview/main.ts + style.css + src/i18n.ts，piCore/protocol 零改动）**：
- 工具组容器化：lastToolGroup 同名合并机制作废，改 toolGroup 容器（.tool-grp = 头行
  「工具 ×N」+ 组体）。toolStart 增 into 参数（历史重绘注入用）；live 归属：进行中组 > 新开
  组；计数在 toolStart 记。组结束时机：appendDelta/appendThink/newLive 调 endToolGroup()
  收成一行头。点击头行开合整组；组内工具行点击开合各自 IN/OUT（全套交互复用）
- 历史重绘同构：toolGroupRun（同名 80 字平铺）删除，连续 toolCall 不分名进
  makeToolGroupShell + toolStart/toolEnd（OUT 截 1000 不持全量引用，老会话内存零增长）
- sticky 追加二：max-height 两行机制作废，改 pinned（IO + 0 高 sentinel 检测，原位不折）
  + line-clamp 3 行（自带 …）+ attr(data-st) chip「展开/收起」（i18n 新 key ×3）。
  内容高原位态 rAF 量一次存 _fullH（不再反复读布局）；每视图一个 IO 实例
- i18n：toolGroup/stickyExpand/stickyCollapse 中英两处

**验证**：npm run compile 全链绿；0.1.30 已打包待实测。

**待实测（用户主验收）**：①流式中连续 bash/edit/bash 混排合成「工具 ×N」一行头，每完成
一个收一个；来思考/正文时整组收成一行头；点头行展开见各工具行，点行再开 IN/OUT；②settled
后历史重绘同构不跳变；③长提问原位全量、滚动钉住后超 3 行折（带 …）+ 右下「展开」chip，
点击展开/收起 chip 随状态换字；短提问钉住不折不显 chip；④回答气泡照常全量。

# 0.1.31：跨消息合组根因修 + 组头状态/嵌套样式（2026-09-20，待实测）

**根因（探针实锤，scripts/probe-toolblocks.mjs 读最新 session jsonl）**：pi **每输出一个
 toolCall 就是一条独立 assistant 消息**（连 bash×3 都是三条）→ live 侧 newLive 把组结束掉
（每工具一组 ×1）；历史侧按消息收集（跨消息不合）。「整复杂了」的回应：纯前端没错，但复杂
度在数据层——跨消息合并必须接住，样式解决不了断组。

**修法**：①live：newLive 不再 endToolGroup（组只被思考块/正文 delta 打断）；②历史：
renderAll 加 crossRun 跨消息状态——toolCall 延续组、bump 计数；非空 think/text/user/
compaction 才断组；makeToolGroupShell 改 bump() 动态计数。

**样式与状态（用户「丑 + 没状态」反馈）**：组体左侧 2px 连线 + 缩进突出嵌套；组内工具行
opacity .8 次级；头行状态三态——运行中 run 脉动点（复用 .tool.run）、结束 ok 绿点、
含错 err 红点（live 经 has-err 标记在 endToolGroup 兑成终态；历史经 crossRun.err()）。

**验证**：npm run compile 全链绿；0.1.31 已打包待实测。

**待实测**：①连续混排工具合成一组实时 ×N 增长，不再每工具一个 ×1；②头行运行中脉动、
结束变绿/错变红；③组体左侧连线嵌套视觉；④settled 后历史不跳变、跨消息合组一致。

# 0.1.32：工具组合方案定稿——回归同名连续合并（2026-09-20 用户拍板，待实测）

**决策链**：0.1.30/31 的「不分名大折叠块 + 跨消息容器」被用户否掉（丑/没状态/整复杂了）；
中间一度误听为「整体砍掉」（edit 未命中未生效，无损害）；最终口径 = **回归 0.1.29 的同名
连续合并**（bash ×N 行），但保留跨消息断组根因修复（空壳 bubble 移除 + DOM 相邻判定）。
探针结论（pi 每 toolCall 一条独立 assistant 消息）继续成立且被同名方案利用：相邻消息的
同名工具行在 DOM 上天然相接可合；思考/正文 bubble 插队即断。

**改动面（webview/main.ts + style.css + i18n.ts）**：
- 删：toolGroup 容器/endToolGroup/makeToolGroupShell/crossRun/into 参数/tool-grp CSS/
  L.toolGroup key（组容器机制全退）
- 留：toolEnd 里同名 DOM 相邻合并（moveChild 零复制、×N 挂 t-name、被合并方全量先展开
  进隐藏盒再释放）、live 与历史共用；完成即收起、长 OUT 截断/全量、思考块限高滚动/贴底
  滚/结束自折、sticky 三态、空壳 bubble 移除全部保持
- 历史 OUT 截 1000 不持全量引用（内存口径不变）

**验证**：npm run compile 全链绿；组容器标识符 grep 零残留；0.1.32 已打包待实测。

**待实测（用户主验收）**：①流式中连续同名工具实时合成「bash ×N / edit ×N」，不同名各自
成行；思考/正文插队断开；②合并不再出现「工具 ×1」假组；③settled 后历史同名合并一致；
④点行开 IN/OUT、长输出全量、思考块限高照旧。

# 0.1.33 事故修复：renderAll 重复段（think/text 双画）（2026-09-20，待实测）

**症状（用户 0.1.32 实测）**：打开面板/settled 后思考块、正文各画两遍；live 流式期间正常。
**根因**：0.1.32 多刀 edit 回归历史分支时，旧版 think/text 段没删净——renderAll 里新旧两段
叠着（独立 if 再判一次），think/text 各命中两次；toolCall 在 else-if 链上只走一次所以正常。
语法合法 → compile 绿，编译器查不出逻辑重复。**模型被切与此无关**（另一条线，待用户补充
现象：切模型前是否有报错提示——疑似 provider 错误自动回退/fork 机制）。
**修法**：删重复段，makeThink 渲染点 grep 验证只剩 1 处。

**教训（入档）**：多刀 edit 改 renderAll 这类长 else-if 链，收尾必须「结构对账」——grep 渲染
点计数（此例：makeThink 应恰 1 处），不能只靠 compile（语法合法但逻辑重复编译器不报）。
与 bindViewEvents 事故同类：「合法存在但没人调/调了两次」编译器不报，只能靠清单核对。

**验证**：makeThink 渲染点 grep=1；compile 全绿；0.1.33 已打包待实测。
**→ 2026-09-20 用户实测通过（settled 后思考/正文各一份）**。同名合并方案定稿。

**待实测**：①settled/重开面板后思考块、正文各只一份；②同名合并照旧；③live 照旧。
