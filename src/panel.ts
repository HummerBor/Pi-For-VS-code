...
function webviewJs(): string {
  ...
  "  var themeEl = document.getElementById('theme');",
    ...
    "  function renderStatus() { var gap = streaming && lastEvt ? Math.round((Date.now() - lastEvt) / 1000) : 0; modeBadge.textContent = modeText; statusEl.textContent = baseStatus + (gap > 8 ? ' · 无响应 ' + gap + 's' : '') + (queueN > 0 ? ' · 排队 ' + queueN + ' 条' : ''); }",
    "  function setBusy(v) {",
    "    streaming = v;",
    ...
    "        setStatus('工作中 (' + Math.floor((Date.now() - busyStart) / 1000) + 's)');",
    "      }, 1000);",
    "    } else {",
    "      setStatus('');",
    "    }",
    ...
    "    if (liveRTimer) return;",
    "    liveRTimer = setTimeout(function () {",
    "      liveRTimer = null;",
    "      if (!liveParts) return;",
    "      for (var k in liveParts) {",
    "        var p = liveParts[k];",
    "        if (p && p.kind === 'text' && !p.done && p.buf.length > p.doneLen) streamTick(p);",
    "      }",
    "      scroll();",
    "    }, 100);",
    "  }",
