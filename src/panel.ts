...
function css(): string {
  return [
    ...
    "",
    "/* ── 底部状态栏 ── */",
    "#hdr-row2 { display: flex; align-items: center; gap: 10px; margin-top: 4px; font-size: 11px; opacity: .75; }",
    "#hdr-row2 #usage { margin-left: auto; white-space: nowrap; font-variant-numeric: tabular-nums; opacity: .8; }",
    "#hdr-row2 .tb-btn { padding: 0; }",
    ".working-row { padding: 4px 14px; font-size: 12px; opacity: .7; }",
    ".working-row.busy { padding: 6px 14px; font-size: 13px; font-weight: 600; opacity: 1; color: var(--vscode-textLink-foreground, #4daafc); animation: wpulse 1.2s ease-in-out infinite; }",
...
function webviewJs(): string {
  return [
    ...
    "  var workingEl = document.createElement('div'); workingEl.className = 'working-row'; workingEl.style.display = 'none';",
    "  function setStatus(t) { if (t) { workingEl.classList.remove('busy'); workingEl.textContent = t; workingEl.style.display = 'block'; messages.appendChild(workingEl); } else if (!streaming) { workingEl.style.display = 'none'; } }",
  ...
  "  var themeEl = document.getElementById('theme');",
    ...
    "    return e;",
    "  }",
    "  function scroll() { if (workingEl.style.display !== 'none') messages.appendChild(workingEl); messages.scrollTop = messages.scrollHeight; }",
    "  function setStatus(t) { if (t) { workingEl.textContent = t; workingEl.style.display = 'block'; messages.appendChild(workingEl); } else { workingEl.style.display = 'none'; } renderStatus(); }",
    "  var queueN = 0;",
    "  var modeText = 'Auto';",
    ...
    "    if (v) {",
    "      workingEl.classList.add('busy');",
    "      messages.appendChild(workingEl);",
    "      var frames = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];",
    "      var fi = 0;",
    "      workingEl.textContent = frames[0] + ' Working…';",
    "      busyTimer = setInterval(function () {",
    "        fi = (fi + 1) % frames.length;",
    "        workingEl.textContent = frames[fi] + ' Working…' + (queueN > 0 ? ' · 排队 ' + queueN + ' 条' : '');",
    "      }, 120);",
    "    } else {",
    "      workingEl.classList.remove('busy');",
