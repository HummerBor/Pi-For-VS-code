...
function css(): string {
  return [
    ...
    "",
    "/* ── 底部状态栏 ── */",
    "#statusline { display: flex; align-items: center; min-height: 16px; padding: 0 14px 2px; font-size: 11px; opacity: .6; }",
    "#statusline #usage { margin-left: auto; white-space: nowrap; font-variant-numeric: tabular-nums; }",
    "",
...
function webviewJs(): string {
  ...
  "  var themeEl = document.getElementById('theme');",
    ...
    "      var frames = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];",
    "      var fi = 0;",
    "      setStatus(frames[0] + ' Working…');",
    "      busyTimer = setInterval(function () {",
    "        fi = (fi + 1) % frames.length;",
    "        setStatus(frames[fi] + ' Working…');",
    "      }, 120);",
    "    } else {",
    "      setStatus('');",
    "    }",
