...
用户报告：安装 0.0.8 + 重载后「没有任何变化」（滚动条仍带箭头的旧样式、上传仍只限图片）。
本轮已加两个验证/兑底手段后出 0.0.9：
1. **版本标记**：底部 bottombar 末尾显示 v0.0.9（host 把 package.json version 经 state 事件传给 webview）。
   下个会话第一件事：让用户看底部有没有 v0.0.9——
   - 没有 → 安装链路问题（VSIX 未真正替换/装错旧副本/扩展目录权限），让用户截扩展面板里本项的版本号
   - 有 → 代码已生效，滚动条问题转向 CSS 兼容性：本轮已加标准属性兑底
     （`* { scrollbar-width: thin; scrollbar-color: … }`，部分 Chromium 里 ::-webkit-scrollbar 不生效，
     标准属性是另一条管线）；仍不行 → Ctrl+Shift+I 看 messages 滚动条伪元素是否命中
2. 上传文件完整版已在 0.0.9 里：所有文件过滤器 + 非图片读文本注入上下文（attachFile→codeCtx）
3. 状态行/头部布局重构也在 0.0.9 里（用户对 0.0.7 的部分验收可能跑的是旧代码，需重验）
